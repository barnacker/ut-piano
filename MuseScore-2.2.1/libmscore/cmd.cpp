...
//                                    undoChangeProperty(oNote, P_ID::STRING, string);
                        ...
                        break;
				  case StaffGroup::UTPIANO_RIGHT_HAND:
				  case StaffGroup::UTPIANO_LEFT_HAND:
...
void Score::moveUp(ChordRest* cr)
	  ...
	  if ((staff->staffType()->group() != StaffGroup::STANDARD && staff->staffType()->group() !=StaffGroup::UTPIANO_RIGHT_HAND  && staff->staffType()->group() != StaffGroup::UTPIANO_LEFT_HAND) ||
          (staves->at(rstaff+staffMove-1)->staffType()->group() != StaffGroup::STANDARD &&
		   staves->at(rstaff+staffMove-1)->staffType()->group() != StaffGroup::UTPIANO_RIGHT_HAND &&
		   staves->at(rstaff+staffMove-1)->staffType()->group() != StaffGroup::UTPIANO_LEFT_HAND)) {
...
void Score::moveDown(ChordRest* cr)
	  ...
	  staff->staffType()->group() != StaffGroup::UTPIANO_RIGHT_HAND &&
	  staff->staffType()->group() != StaffGroup::UTPIANO_LEFT_HAND)||
		(staves->at(staffMove+rstaff+1)->staffType()->group() != StaffGroup::STANDARD &&
		 staves->at(staffMove+rstaff+1)->staffType()->group() != StaffGroup::UTPIANO_RIGHT_HAND &&
		 staves->at(staffMove+rstaff+1)->staffType()->group() != StaffGroup::UTPIANO_LEFT_HAND)) {
            qDebug("User attempted to move a note from/to a staff which does not use standard notation - ignoring.");
            }
      else  {
