...
const char StaffType::groupNames[STAFF_GROUP_MAX][STAFF_GROUP_NAME_MAX_LENGTH] = {
      ...
      QT_TRANSLATE_NOOP("Staff type group name", "UTP-R"),
      QT_TRANSLATE_NOOP("Staff type group name", "UTP-L")
      };

const QString StaffType::fileGroupNames[STAFF_GROUP_MAX] = { "pitched", "percussion", "tablature","UTP-R", "UTP-L" };
...
void StaffType::write(Xml& xml) const
      ...
      if (_group == StaffGroup::STANDARD || _group == StaffGroup::PERCUSSION || _group == StaffGroup::UTPIANO_RIGHT_HAND || _group == StaffGroup::UTPIANO_LEFT_HAND) {
            if (!_genKeysig)
...
void StaffType::read(XmlReader& e)
      ...
      if (group == fileGroupNames[(int)StaffGroup::TAB])
 	  ...
 	  else if (group == fileGroupNames[(int)StaffGroup::UTPIANO_RIGHT_HAND])
            _group = StaffGroup::UTPIANO_RIGHT_HAND;
	  else if (group == fileGroupNames[(int)StaffGroup::UTPIANO_LEFT_HAND])
            _group = StaffGroup::UTPIANO_LEFT_HAND;
      else {
...
qreal StaffType::doty2() const
      ...
      switch(_lines) {
            ...
            case 11:
                  return _lineDistance.val() * -0.5;
            case 12:
                  return _lineDistance.val() * 11.5;
            default:
...
//                 group               xml-name,         human-readable-name         lin dist  clef   bars stemless time      duration font     size off genDur     fret font          size off  duration symbol repeat        thru  minim style                  onLin  rests  stmDn  stmThr upsDn  sTFing   nums bkTied
         ...
         StaffType(StaffGroup::UTPIANO_RIGHT_HAND, "UTP-R", QObject::tr("UT-Piano-RightHand"),12,1, true, true, false, true, true, true),
         StaffType(StaffGroup::UTPIANO_LEFT_HAND, "UTP-L", QObject::tr("UT-Piano-LeftHand"), 11,1, true, true, false, true, true, true)
