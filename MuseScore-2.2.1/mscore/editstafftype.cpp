...
const char* g_groupNames[STAFF_GROUP_MAX] = {
	  ...
	  QT_TRANSLATE_NOOP("staff group header name", "UTPIANO RIGHT STAFF"),
	  QT_TRANSLATE_NOOP("staff group header name", "UTPIANO LEFT STAFF")
      };
   ...
   : QDialog(parent)
      ...
      for (const StaffType& t : StaffType::presets()) {
            if ( (t.group() == StaffGroup::STANDARD && bStandard)
						...
						|| (t.group() == StaffGroup::UTPIANO_RIGHT_HAND)
						|| (t.group() == StaffGroup::UTPIANO_LEFT_HAND)
						)
...
void EditStaffType::setFromDlg()
	  ...
	  staffType.group() == StaffGroup::UTPIANO_RIGHT_HAND ||
	  staffType.group() == StaffGroup::UTPIANO_LEFT_HAND ) {
      ...
      for (int idx = 1; ; ++idx) {
            switch (group) {
                  ...
                  case StaffGroup::TAB:
                        ...
                        break;
                  case StaffGroup::UTPIANO_RIGHT_HAND:
                        name = QString("UT-Piano-RightHand-%1 [*]").arg(idx);
                        break;
                  case StaffGroup::UTPIANO_LEFT_HAND:
                      name = QString("UT-Piano-LeftHand-%1 [*]").arg(idx);
                        break;
                  }
            bool found = false;
