...
void StaffListItem::initStaffTypeCombo(bool forceRecreate)
      ...
      for (const StaffType& st : StaffType::presets()) {
            if ( (st.group() == StaffGroup::STANDARD && (!canUsePerc))    // percussion excludes standard
                 ...
                 || (st.group() == StaffGroup::UTPIANO_RIGHT_HAND && (!canUsePerc))
                 || (st.group() == StaffGroup::UTPIANO_LEFT_HAND && (!canUsePerc))
                 ) {
      ...
      for (int i = 0; i < childCount(); ++i) {
            ...
            switch (stfType->group()) {
                  ...
                  case StaffGroup::UTPIANO_RIGHT_HAND:
                        clefType = ClefTypeList(ClefType::UTPIANO_RIGHT);
                        break;
                  case StaffGroup::UTPIANO_LEFT_HAND:
                        clefType = ClefTypeList(ClefType::UTPIANO_LEFT);
                        break;
                  }
...
//   hasUTPianoStaff
//---------------------------------------------------------
bool InstrumentsWidget::hasUTPianoStaff()
{
    bool found = false;
    // Traverse tree to check if there is a UTPiano Staff
    QTreeWidget* pl = partiturList;
    QTreeWidgetItem* item = 0;
    for (int idx = 0; (item = pl->topLevelItem(idx)); ++idx)
    {
          ...
          for (int cidx = 0; (ci = pli->child(cidx)); ++cidx)
          {
                if(ci->isHidden())
                  continue;
                StaffListItem* sli = (StaffListItem*)ci;
                if (sli->staffType()->group() == StaffGroup::UTPIANO_RIGHT_HAND ||
                sli->staffType()->group() == StaffGroup::UTPIANO_LEFT_HAND)
                {
                    found = true;
                    break;
                }
