      ...
      if (ct._concertClef == ClefType::INVALID) {
            switch(_staffType.group()) {
                  case StaffGroup::UTPIANO_RIGHT_HAND:
                    ct = ClefTypeList(ClefType::UTPIANO_RIGHT);
                    break;
                  case StaffGroup::UTPIANO_LEFT_HAND:
                    ct = ClefTypeList(ClefType::UTPIANO_LEFT);
                    break;
                  case StaffGroup::TAB:
...
// WARNING: following statement assumes number of staff lines to be correctly set
            ...
            else if (tag == "distOffset")
            {
                ...
                if (_staffType.group() == StaffGroup::UTPIANO_LEFT_HAND)
                {
                    double doffset = e.readDouble();
                    doffset *= score()->spatium();
                    _userDist = 0;
                } else
                {
