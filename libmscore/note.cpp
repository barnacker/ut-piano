...
#ifndef DISABLE_UTPIANO
            // Draw the 45 degree little line to the left of the note for all notes that
            // are over a line
             if (staff()->isUtPianoStaff() && (_line % 2) == 0)
             {
                qreal sp = spatium();
                qreal x1 = bbox().x();
                qreal y1 = bbox().y();
                float lw = score()->styleS(StyleIdx::staffLineWidth).val() * sp * 2.0;
                painter->save();
                painter->setPen(QPen(c, lw, Qt::SolidLine, Qt::SquareCap));
                painter->drawLine(QLineF(x1-(sp * 0.25), y1, x1+(sp * 0.1), y1+(sp * 0.5)));
                painter->restore();
             }
#endif
            }
...
void Note::endDrag()
      ...
      _lineOffset = 0;
#ifndef DISABLE_UTPIANO
            // For UT-PIANO set the steam up if pitch greater or equel to 60 (Middle C); steam down otherwise.
...
void Note::updateRelLine(int relLine, bool undoable)
      ...
      int line;
 #ifndef DISABLE_UTPIANO
      // Update line in UT-PIANO
      if (clef == ClefType::UTPIANO_RIGHT || clef == ClefType::UTPIANO_LEFT ||
          clef == ClefType::UTPIANO_RIGHT_2 || clef == ClefType::UTPIANO_LEFT_2 ||
          clef == ClefType::UTPIANO_RIGHT_3 || clef == ClefType::UTPIANO_LEFT_3 )
      {
          // Re-adjust lines
          line = pitch2line(_pitch, clef);
      }
      else
 #endif
      {
