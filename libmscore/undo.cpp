...
void Score::undoAddCR(ChordRest* cr, Measure* measure, int tick)
      ...
      foreach (Staff* staff, ostaff->staffList()) {
            ...
            // Change steam when pasting depending on the staff
            ClefType clef   = staff->clef(tick);
            if (newcr->type() == Element::Type::CHORD) {
                  Chord* chord1 = static_cast<Chord*>(newcr);

                  int countUp = 0;
                  int countDown = 0;
                  foreach(Note* note, chord1->notes()) {
                      if (note->pitch()>=60)
                          countUp++;
                      else
                          countDown++;
