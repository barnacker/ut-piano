...
void Dynamic::layout()
      ...
      for (int voice = 0; voice < VOICES; ++voice) {
        ...
        if (score()->staff(0) &&  score()->staff(0)->isUtPianoStaff())
        {
            if (score()->staff(staffIdx())->isUtPianoRightStaff())
            {
                 rypos() += score()->staff(staffIdx())->height()*1.55;
            }
            else
            {
                 rypos() += score()->staff(staffIdx())->height()/2.0;
            }
        }
      if (isPasted())
      {
